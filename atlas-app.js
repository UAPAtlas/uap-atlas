...
function caseCard(c){const active = c.id===state.selectedCaseId ? 'active' : ''; const status=markerStatus(c); return `<div class="case-row ${active}" data-id="${c.id}" role="button" tabindex="0" aria-label="Select ${esc(c.title)}, ${esc(c.date)}"><div class="file-main"><div class="t case-row-title">${esc(c.title)}</div><div class="m case-row-meta">${esc(c.location)} · ${esc(c.agency)}</div></div><div class="file-id"><div class="id">${esc(c.id)}</div><small>${esc(c.date)}</small></div><div class="row-actions"><div class="pill ${statusPill[status]}">${esc(c.mode)}</div></div></div>`;}
...
function visualRoleLabel(item){
  const raw=String(item?.visualType||item?.role||'visual').toLowerCase();
  const status=String(item?.evidenceStatus||'');
  let label='Visual reference';
  if(/site-context|context/.test(raw)) label='Location context · Not event imagery';
  else if(/reconstruction|illustration/.test(raw)) label='Reconstruction · Not event imagery';
  else if(/sensor|radar|infrared|thermal|flir|video-frame/.test(raw)) label='Sensor / released frame';
  else if(/document|record|report|press|cover|routing|page|official|intelligence/.test(raw)) label='Documentary record';
  else if(item?.role==='reference') label='Reference image · Not event imagery';
  else if(item?.role==='evidence') label='Evidence image';
  if(/not (direct )?(imagery|image)|not event|context image only|reconstruction/i.test(status) && !/Not event imagery/i.test(label)) label += ' · Not event imagery';
  return label;
}
function sourceBoundarySummary(c){
  const records=lensRecords(c);
  if(records.length) return `${records.length} mapped source record${records.length===1?'':'s'} · ${lensCustody(c,records).label}`;
  return c.sourceQuality||c.sourceLabel||'Public source boundary is described in the source tabs.';
}
...
function carouselMediaHtml(item){const badge=`<span class="carousel-role-badge">${esc(visualRoleLabel(item))}</span>`;return item.kind==='video'?`<video class="carousel-media" controls preload="metadata" src="${esc(item.src)}"></video>${badge}`:`<div class="carousel-media-shell" data-lightbox-trigger role="button" tabindex="0" aria-label="Open ${esc(item.label)} in image viewer">${badge}<img class="carousel-media" src="${esc(item.src)}" alt="${esc(item.label)}" data-lightbox-src="${esc(item.src)}" loading="eager" decoding="async"></div>`;}
